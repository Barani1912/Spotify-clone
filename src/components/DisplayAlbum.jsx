...
const DisplayAlbum = () => {
  ...
  return (
    <>
      ...
      <div className='mt-10 ml-7 flex gap-8 flex-col md:flex-row md:items-end'>
      ...
      <div className='grid grid-cols-3 sm:grid-cols-4  pt-7 pb-2 pr-2 text-[#a7a7a7]'>
        <p className='pl-4'><b className='mr-4'>#</b>Title</p>
        <p className='text-center'>Album</p>
        <p className='hidden text-center sm:block'>Date Added</p>
        <img className='m-auto w-4' src={assets.clock_icon} alt='' />
      </div>
      <hr className='ml-4 mr-4 opacity-20'/>
      {
        songsData.map((item,index)=>(
          <div onClick={()=>playWithId(item.id)} key={index} className='grid grid-cols-3 sm:grid-cols-4 gap-2 p-4 items-center text-[#a7a7a7] hover:bg-[#ffffff2b] cursor-pointer'>
